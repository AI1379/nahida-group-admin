...
> [!NOTE]
> 🚧 **项目状态：开发中。** 已基于 NoneBot2 搭好骨架，并完成首个功能 **🎖️ 自助派发头衔**（OneBot 11 后端已验证）。其余功能仍在开发中，下文标注 🚧 的部分为目标设计，配置项与命令格式可能调整。欢迎通过 Issue / PR 参与。
...
- **🎖️ 自助派发头衔** ✅
  群成员通过命令自助设置自己的群头衔（群专属头衔），无需管理员逐个手动操作。可配置头衔长度、敏感词与冷却时间等限制。

- **📨 入群审批通知** 🚧
  当有人申请加群时，机器人在群内推送一条包含申请人信息与验证消息的通知。**任意管理员**只需「回复」这条通知消息即可决定通过或拒绝，审批结果会同步反馈到群内。

- **🏺 陶片放逐（Ostracism）** 🚧
  借鉴古雅典「陶片放逐制」的群内投票踢人机制：由成员发起对某人的放逐投票，在限定时间窗内达到票数阈值即自动将其移出群聊。阈值、时限、发起权限均可配置。

- **🔇 自助禁言** 🚧
  允许群成员在规则约束下对特定成员发起禁言。典型场景：某个挂着特定头衔（例如头衔为 `bot` 的机器人小号）的成员，允许普通群员对其自助禁言。可基于「头衔 / 角色」配置「谁可以禁言谁、禁言多久」。

- **👉 戳一戳与命令文本互动** 🚧
  响应戳一戳（双击头像 / Poke）事件，以及自定义的命令文本，返回配置好的回复内容，适合做彩蛋、帮助菜单或快捷指令。
...
| `/放逐 @某人` | 发起陶片放逐投票 | 视配置而定 | 🚧 计划中 |
| `/禁言 @某人 <时长>` | 在规则允许范围内自助禁言 | 视配置而定 | 🚧 计划中 |
| 戳一戳机器人 | 触发戳一戳回复 | 所有成员 | 🚧 计划中 |
| 自定义命令文本 | 触发对应的文本回复 | 所有成员 | 🚧 计划中 |
